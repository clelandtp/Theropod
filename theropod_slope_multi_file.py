...
def stori_slope_calculator(f): 
    ...
    print(os.path.splitext(os.path.basename(f))[0], np.size(mznew[peak_idx]))
    print('test')
    pslopes1 = []
    pslopes2 = []
    pslopes3 = []
    ...
    slopesdf1['Scan'] = os.path.splitext(os.path.basename(f))[0]
    slopesdf1['Ions'] = np.size(mznew[peak_idx])
    slopesdf1.to_pickle(f+"_stori.pkl", compression = 'xz')

def main():
    with concurrent.futures.ProcessPoolExecutor(max_workers=19) as executor:
         executor.map(stori_slope_calculator, pkl_files)
